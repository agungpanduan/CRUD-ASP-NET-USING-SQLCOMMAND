    ...
    if (typeof returnResult.Result != "undefined" && typeof returnResult.ErrMesgs != "undefined") {
        if (returnResult.Result == returnResult.ValueSuccess) {
            if (!isNotShowSuccessMesg) {
                var mesg = "Proses " + processName + " selesai dengan sukses";
                if (typeof returnResult.SuccMesgs != "undefined" && returnResult.SuccMesgs != null) {
                    for (i = 0; i < returnResult.SuccMesgs.length; i++) {
                        mesg += "<br/>" + (returnResult.SuccMesgs[i] != null ? returnResult.SuccMesgs[i].replace(/\\n/g, "<br/>") : "");
                    }
                }
            ...
            if (returnResult.ErrMesgs != null) {
                var errMesg = "Proses " + processName + " selesai dengan kesalahan : <br/>";
                if (typeof returnResult.ProcessId != "undefined" && returnResult.ProcessId != null) {
                ...
                for (i = 0; i < returnResult.ErrMesgs.length; i++) {
                    errMesg += (returnResult.ErrMesgs[i] != null ? returnResult.ErrMesgs[i].replace(/\\n/g, "<br/>") : "") + "<br/>";
                }
                $.msgBox({
                    title: "ERROR",
                    type: "error",
...
$(document).ready(function () {
    $(".enterOnSearch").keyup(function (event) {
        if (event.keyCode == 13) {
            onSearchCriteria(1);
        }
    });

    $(".cmbChangeOnSearch").bind("change keyup", function (event) {
        if (event.keyCode == 13) {
            onSearchCriteria(1);
        }
    });
...
function onGenbaResultNG() {
    $('.tdGenbaResult').each(function () {
        if ($(this).attr('data-reviewResultCd') == 'SGR_NG' || $(this).attr('data-reviewResultCd') == 'SRR_NG') {
            $(this).css({ "background-color": "#FF0000", "color": "#000000", "align": "center" });
        }
    });
}

function ParseJsonObject(ostr) {
    var sparso = unescape(ostr);
    sparso = sparso.replace(/\\/g, "\\\\");
    sparso = sparso.replace(/&quot;/g, '"');
    var oParsed = new Object();
    try {
        oParsed = JSON.parse(sparso);
    } catch (e) {
        console.log(e);
    }
    return oParsed;
...
function AsInt(strNum) { return (!(Number.parseInt(strNum) == NaN)) ? Number.parseInt(strNum) : null; }
